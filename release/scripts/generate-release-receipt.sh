...
if [ "${GLYPHCULL_RECEIPT_FULL:-0}" = "1" ]; then
  ...
  if ./scripts/ci-smoke.sh >/dev/null 2>&1; then RESULT_CONF=pass; else RESULT_CONF=fail; fi
fi
...
const receipt = {
  ...
  commands: {
    ...
    conformance: './scripts/ci-smoke.sh (compiler self-check) + (glyphcull-demo) ./scripts/run-conformance.sh',
